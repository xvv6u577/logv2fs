...
import { TrafficByPeriod } from "../types";

interface Props {
	data?: TrafficByPeriod[];
	limit?: number;
	by: string;
}

const TrafficTable:FunctionComponent<Props> = ({ data, limit, by }) => {
	return (
									...
									<tr key={index} className="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
										...
										<td className="py-4 px-2">{item.amount && Object.entries(item.used_by_domain).map(([key, value]) => {
											return (
												...
												// 		{key}:{" "}
												// 		<span className="bg-green-100 text-green-800 text-sm font-medium mr-2 px-0 py-0.5 rounded dark:bg-green-200 dark:text-green-900">
												// 			{formatBytes(value)}
												// 		</span>
												// 	</span><br />
												// </div>
