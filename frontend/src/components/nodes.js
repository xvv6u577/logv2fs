...
function Nodes() {
    ...
    const [singboxNodes, sestSingboxNodes] = useState([]);
    ...
    useEffect(() => {
        axios
            ...
            .then((response) => {
                sestSingboxNodes(response.data);
            })
            .catch((err) => {
        ...
        axios
            ...
            .then((response) => {
                setMonitoredDomains(response.data);
            })
            .catch((err) => {
                dispatch(alert({ show: true, content: err.toString() }));
            });
                                    ...
                                    <div className="p-4 lg:w-1/3" key={index} >
                                        <div className="h-full bg-gray-800 bg-opacity-40 px-8 pt-10 pb-10 rounded-lg overflow-hidden text-center relative hover:bg-gray-700">
                                            <button className="absolute top-0 right-0 p-2"
                                                onClick={() => {
                                                    var tempDomains = monitoredDomains.filter(item => item.domain !== domain.domain);
                                                    setMonitoredDomains(tempDomains);
                                                }}
                                            >
                                    ...
                                    <button type="button"
                                        onClick={() => {
                                            if (newdomain.length > 0 && newRemark.length > 0) {
                                                var tempDomains = monitoredDomains.filter(item => item.domain === newdomain);
                                                if (tempDomains.length === 0) {
                                                    setMonitoredDomains([...monitoredDomains, { domain: newdomain, remark: newRemark, days_to_expire: -1, expired_date: "" }]);
                                                }
                                                updateNewdomain("");
                                                updateNewRemark("");
                                            } else {
                            ...
                            >
                                ...
                                <span className="flex md:justify-start justify-center w-full md:w-3/5">
                                    ...
                                    <span className="inline-flex w-24 bg-indigo-100 text-indigo-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-indigo-200 dark:text-indigo-900">
                                        {node?.daily_logs?.length > 0 ? formatBytes(node.daily_logs?.[0]?.traffic) : "0"}
                                    </span>
                                    This Month:
                                    <span className="inline-flex w-24 bg-indigo-100 text-indigo-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-indigo-200 dark:text-indigo-900">
                                        {node?.monthly_logs?.length > 0 ? formatBytes(node.monthly_logs?.[0]?.traffic) : "0"}
                                    </span>
                                    This Year:
                                    <span className="inline-flex w-24 bg-indigo-100 text-indigo-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-indigo-200 dark:text-indigo-900">
                                        {node?.yearly_logs?.length > 0 ? formatBytes(node.yearly_logs?.[0]?.traffic) : "0"}
                                    </span>
                                </span>
                                <svg
                                    onClick={() => {
                            ...
                            <div className="my-4">
                                ...
                                <div className="overflow-x-auto relative shadow-md sm:rounded-lg">
                                    <table className="table-auto w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                        ...
                                        <tbody>
                                            {node?.monthly_logs?.sort((a, b) => b.month - a.month)
                                                .slice(0, 5)
                                                .map((item, index) =>
                            ...
                            <div className="my-4">
                                ...
                                <div className="overflow-x-auto relative shadow-md sm:rounded-lg">
                                    <table className="table-auto w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                        ...
                                        <tbody>
                                            {node?.daily_logs?.sort((a, b) => b.date - a.date)
                                                .slice(0, 14)
                                                .map((item, index) =>
                                                (<tr key={index} className="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                                    <td className="py-4 px-2">{index + 1}</td>
