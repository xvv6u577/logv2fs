...
// Inbounds 返回当前 box 持有的所有 inbound 适配器。
// logv2fs 用于在运行时遍历 inbound 做用户增/删（patch by logv2fs）。
func (s *Box) Inbounds() []adapter.Inbound {
	return s.inbounds
}

// Inbound 按 tag 查找单个 inbound，找不到返回 nil（patch by logv2fs）。
func (s *Box) Inbound(tag string) adapter.Inbound {
