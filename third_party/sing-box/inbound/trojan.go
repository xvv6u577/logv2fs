...
// AddUser 运行时追加一个 Trojan 用户（patch by logv2fs）。语义见 VLESS.AddUser。
// 注意：底层 trojan.Service.UpdateUsers 会在密码冲突时返回 ErrUserExists，
// 调用方需自行保证用户密码不重复。
func (h *Trojan) AddUser(user option.TrojanUser) error {
	h.usersAccess.Lock()
...
// RemoveUser 运行时按 Name 移除一个 Trojan 用户（patch by logv2fs）。
// 以 Password 置空作为墓碑标记，rebuild 时跳过；幂等。
func (h *Trojan) RemoveUser(name string) error {
	h.usersAccess.Lock()
	defer h.usersAccess.Unlock()
...
// rebuildServiceLocked 重建底层 trojan.Service 的用户表，调用方持有写锁。
func (h *Trojan) rebuildServiceLocked() error {
	indices := make([]int, 0, len(h.users))
	passwords := make([]string, 0, len(h.users))
	for i, u := range h.users {
		if u.Password == "" { // tombstone：跳过，避免登记"空密码"用户造成认证漏洞
			continue
		}
